...
const Trending = () => {
 ...
 const [searchTerm, setSearchTerm] = useState('beyonce')
const [pageCount, setPageCount] = useState(15);
const [currentPage, setCurrentPage] = useState(1)

 const getArtists = async () => {
   setIsLoading(true);
   const searchParam = searchTerm ? searchTerm : 'beyonce'
   try {
     const { data } = await axios(
         `http://127.0.0.1:8000/api/top-albums-by-artist?artist=${searchParam}&page=${currentPage}`
       );
       setIsLoading(false);
       setData(data.topalbums.album);
       setPageCount(data.topalbums["@attr"].totalPages)
     } catch (error) {
       setIsLoading(false);
 ...
 const handleChange = (e) => {
  ...
  return (
    <div ref={divRef} className='flex flex-col'>
      <Hero title={`Discover Best Albums`} />
      ...
      <div className='grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
        {data.map((album, i) => {
          return <Card key={i} album={album} i={i} />;
        })}
      </div>
    </div>
  );
